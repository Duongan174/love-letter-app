// app/api/admin/templates/upload/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@/lib/supabase/server';

/**
 * POST /api/admin/templates/upload
 * Upload template file to Supabase Storage (server-side để bypass RLS)
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // 5. Validate file size (max 50MB for video, 10MB for image)
    ...
    return NextResponse.json({
      success: true,
      url: publicUrl,
      mediaType: getMediaType(publicUrl),
      path: filePath,
    });
  } catch (error: any) {
